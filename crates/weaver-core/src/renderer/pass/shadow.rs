...
impl UniqueMeshes {
    pub fn gather(&mut self, world: &World) {
        let query = world.query(&Query::new().read::<Mesh>().read::<GlobalTransform>());
...
impl Pass for OmniShadowRenderPass {
    ...
    fn prepare(&self, world: &World, renderer: &Renderer) -> anyhow::Result<()> {
        ...
        for (i, point_light_entity) in point_lights.iter().enumerate() {
            let point_light = point_lights.get::<PointLight>(point_light_entity).unwrap();
    ...
    fn render(
        ...
        let lights = world.query(&Query::new().read::<PointLight>());
        for (i, light_entity) in lights.iter().enumerate() {
            let light = lights.get::<PointLight>(light_entity).unwrap();
