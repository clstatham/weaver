...
pub fn bsp_file(input: &[u8]) -> IResult<&[u8], BspFile> {
    ...
    let (_, entities) = crate::parser::entities(entity_bytes)?;
    let (_, textures_vec) = count(
        crate::parser::texture,
        textures.length as usize / Texture::size(),
    )(textures_bytes)?;
    let (_, planes_vec) =
        count(crate::parser::plane, planes.length as usize / Plane::size())(planes_bytes)?;
    let (_, nodes_vec) =
        count(crate::parser::node, nodes.length as usize / Node::size())(nodes_bytes)?;
    let (_, leafs_vec) =
        count(crate::parser::leaf, leafs.length as usize / Leaf::size())(leafs_bytes)?;
    let (_, leaf_faces_vec) = count(
        crate::parser::leaf_face,
        leaf_faces.length as usize / LeafFace::size(),
    )(leaf_faces_bytes)?;
    let (_, leaf_brushes_vec) = count(
        crate::parser::leaf_brush,
        leaf_brushes.length as usize / LeafBrush::size(),
    )(leaf_brushes_bytes)?;
    let (_, models_vec) =
        count(crate::parser::model, models.length as usize / Model::size())(models_bytes)?;
    let (_, brushes_vec) = count(
        crate::parser::brush,
        brushes.length as usize / Brush::size(),
    )(brushes_bytes)?;
    let (_, brush_sides_vec) = count(
        crate::parser::brush_side,
        brush_sides.length as usize / BrushSide::size(),
    )(brush_sides_bytes)?;
    let (_, verts_vec) =
        count(crate::parser::vert, verts.length as usize / Vert::size())(verts_bytes)?;
    let (_, mesh_verts_vec) = count(
        crate::parser::mesh_vert,
    ...
    let (_, faces_vec) =
        count(crate::parser::face, faces.length as usize / Face::size())(faces_bytes)?;
    let (_, lightmaps_vec) = count(
        crate::parser::lightmap,
        lightmaps.length as usize / Lightmap::size(),
    )(lightmaps_bytes)?;
    let (_, light_vols_vec) = count(
        crate::parser::light_vol,
        light_vols.length as usize / LightVol::size(),
    )(light_vols_bytes)?;

    // let textures_vec = take_while_ok(crate::parser::texture)(textures_bytes)?.1;
    // let planes_vec = take_while_ok(crate::parser::plane)(planes_bytes)?.1;
    // let nodes_vec = take_while_ok(crate::parser::node)(nodes_bytes)?.1;
    // let leafs_vec = take_while_ok(crate::parser::leaf)(leafs_bytes)?.1;
    // let leaf_faces_vec = take_while_ok(crate::parser::leaf_face)(leaf_faces_bytes)?.1;
    // let leaf_brushes_vec = take_while_ok(crate::parser::leaf_brush)(leaf_brushes_bytes)?.1;
    // let models_vec = take_while_ok(crate::parser::model)(models_bytes)?.1;
    // let brushes_vec = take_while_ok(crate::parser::brush)(brushes_bytes)?.1;
    // let brush_sides_vec = take_while_ok(crate::parser::brush_side)(brush_sides_bytes)?.1;
    // let verts_vec = take_while_ok(crate::parser::vert)(verts_bytes)?.1;
    // let mesh_verts_vec = take_while_ok(crate::parser::mesh_vert)(mesh_verts_bytes)?.1;
    // let effects_vec = take_while_ok(crate::parser::effect)(effects_bytes)?.1;
    // let faces_vec = take_while_ok(crate::parser::face)(faces_bytes)?.1;
    // let lightmaps_vec = take_while_ok(crate::parser::lightmap)(lightmaps_bytes)?.1;
    // let light_vols_vec = take_while_ok(crate::parser::light_vol)(light_vols_bytes)?.1;
    let (_, vis_data) = crate::parser::vis_data(vis_data_bytes)?;
...
mod tests {
    ...
    #[test]
    fn test_bsp_file() {
        let bytes = include_bytes!("../../../assets/maps/ctf_gate1.bsp");
        let (_, _bsp_file) = bsp_file(bytes).unwrap();
    }
}
